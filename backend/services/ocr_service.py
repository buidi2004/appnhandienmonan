...
def process_image_for_ingredients(image_path: str) -> List[str]:
    ...
    Returns:
        List[str]: Danh sách nguyên liệu nhận diện được
    """
    try:
...
"""
        ...
        logger.info(f"Recognized {len(ingredients)} ingredients")
        return ingredients
...
def process_image_for_all(image_path: str, health_profile: Optional[Dict] = None) -> Dict:
    """
    ...
    Returns:
        Dict: Dictionary chứa ingredients và recipes
    """
    try:
        img = Image.open(image_path)
  ...
  "recipes": [
    {{
      ...
      "tips": "Mẹo nấu ăn"
        ...
        logger.error(f"Error processing image for all: {str(e)}", exc_info=True)
