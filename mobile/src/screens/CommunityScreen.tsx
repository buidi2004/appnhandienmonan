    ...
    <View style={[styles.postCard, { backgroundColor: colors.card, borderBottomColor: colors.border }]}>
      ...
      <View style={styles.postHeader}>
        ...
        <View style={styles.userInfo}>
          <Text style={[typography.h3, { color: colors.text }]}>{item.user.name}</Text>
      ...
      {/* Post Content */}
      <Text style={[typography.body, { color: colors.text, paddingHorizontal: 16, marginBottom: 12 }]}>
        {item.caption}
      </Text>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        <Text style={[typography.h2, { color: colors.text }]}>Cộng đồng</Text>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <>
            {/* Top Trending Chefs */}
            <View style={[styles.trendingHeader, { backgroundColor: colors.background }]}>
              <Text style={[typography.caption, { color: colors.textSecondary, fontWeight: 'bold', marginLeft: 16, marginBottom: 12 }]}>
                ĐẦU BẾP ĐANG LÊN
              </Text>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 16 }}>
                {[1,2,3,4,5,6].map(i => (
                  <View key={i} style={styles.trendingChef}>
                    ...
                    <Text style={[typography.caption, { color: colors.text, marginTop: 4, fontSize: 10 }]} numberOfLines={1}>
                      User {i}
                    </Text>
                  </View>
                ))}
            ...
            <View style={[styles.createPost, { backgroundColor: colors.card, borderBottomColor: colors.border }]}>
              ...
              <TouchableOpacity style={[styles.postInput, { backgroundColor: colors.background, borderRadius: 20 }]}>
                <Text style={{ color: colors.textSecondary }}>Hôm nay bạn nấu món gì ngon?</Text>
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  postCard: { marginBottom: 8, borderBottomWidth: 1 },
